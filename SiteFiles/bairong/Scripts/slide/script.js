...
var epidiascope = {
	...
	select : function(num,type){
		...
		if (slide.isIE) {
			...
			this.ImgObj1.src = '/sitefiles/bairong/icons/s.gif';
			if (!isIE6) this.ImgObj1.filters[0].Apply();
			
			this.ImgObj1.src = this.filmstrips[num].src;
			if (!isIE6) this.ImgObj1.filters[0].Play();
		}else{
			this.ImgObj1.src = this.filmstrips[num].src;
			for(i = 0;i <= 3;i ++){
...
epidiascope.speedBar = { //速度条
	...
	init : function(){
		...
		slide.$(this.infoId).innerHTML = this.grade + "秒";
