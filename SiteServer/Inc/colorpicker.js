...
{
    $(attributeName + '_highlight_colorselect').selectedIndex=0;
    $(attributeName + '_highlight_colorselect').style.background='#FFFFFF';
	SelRGB = e.title;
	EndColor();
}
...
function ColorPickerOK()
{
    ...
    $(attributeName + '_formatColor').value=selectcolor;
    obj=$(attributeName + '_formatColor');
    $(attributeName + '_s_bgcolor').style.background=selectcolor;
	obj.focus();
	obj.select();
...
{
	document.getElementById(attributeName + '_formatColor').value='';
	document.getElementById(attributeName + '_s_bgcolor').style.background='#FFFFFF';
	HideColorPanel();	
}
...
{ 
   	...
   	var p = getposition($(attributeName + '_formatColor'));
  	//alert(p['x']);
...
function selectoptioncolor(obj, val)
{
    //alert(obj.value);
	attributeName = val;
      $(attributeName + '_formatColor').value=obj.value;
      $(attributeName + '_s_bgcolor').style.background=obj.value;
      //if(navigator.appName.indexOf("Explorer") <0)
      {
          $(attributeName + '_highlight_colorselect').style.background=obj.value;
      }
      $(attributeName + '_formatColor').focus();
}
